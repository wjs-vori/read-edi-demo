...
const DEFAULT_850_PAYLOAD = fs.readFileSync(
  "./src/resources/X12/5010/850/input.edi",
  "utf-8"
);
const DEFAULT_855_PAYLOAD = fs.readFileSync(
  "./src/resources/X12/5010/855/input.edi",
  "utf-8"
);
...
(async () => {
  ...
  console.log(
    `Uploading ${DEFAULT_FILE_PAYLOADS.length} documents to ${bucketName} with loop count: ${loopCount}\n`
  );

  const iterations = Array.from(Array(loopCount).keys());
  const promises = iterations.map(async (iteration) => {
    return await Promise.all(
      DEFAULT_FILE_PAYLOADS.map(async (payload, index) => {
